...
def visualize(args, name, batch_data, infer_model, case, nSample):
    ...
    cond_style_imgs = batch_data["condition_image_style"].cuda()
    print("cond style imgs shape:", cond_style_imgs.shape)
    ...
    cond_img_style = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(cond_style_imgs))
    cond_img_style = cond_img_style.repeat(nSample, 1, 1, 1)
    cond_img_style_cat = [cond_img_style]
    ...
    if 'extra_appearance' in batch_data:
        ...
        m_cond_img_style = batch_data['extra_appearance_style'] # assume only one batch per inference.
        m_cond_img_style = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(m_cond_img_style.cuda()))
        m_cond_img_style = m_cond_img_style.repeat(nSample, 1, 1, 1)
        more_cond_imgs_style = m_cond_img_style#.append([m_cond_img_style])

    # conditions = torch.roll(conditions, shifts=8, dims=0)
    # fea_condtion = torch.roll(fea_condtion, shifts=8, dims=0)  

    """
    last_view_start = len(conditions) - nSample 
    conditions = torch.cat([conditions[last_view_start:], conditions], dim=0)
    fea_condtion = torch.cat([fea_condtion[last_view_start:], fea_condtion], dim=0)
    """
    motion_attn_bank = []

    if not args.save_video:
        image_name = batch_data['image_name'][0].split('.')[0]
        os.makedirs(f"{args.local_image_dir}/{image_name}", exist_ok=True)

    style_mask = batch_data['style_mask'].cuda() if 'style_mask' in batch_data else None
    ...
    for i in range(conditions.shape[0] // nSample):
        ...
        if args.denoise_from_guidance:
            ...
            c = {"c_concat": [conditions[i*nSample : i*nSample+nSample]], "c_crossattn": [c_cross], "image_control": cond_img_cat, 'feature_control':fea_map_enc, "image_control_style": cond_img_style_cat, "style_mask": style_mask}
        else:
            c = {"c_concat": [conditions[i*nSample: i*nSample+nSample]], "c_crossattn": [c_cross], "image_control": cond_img_cat, "image_control_style": cond_img_style_cat, "style_mask": style_mask}
        if args.control_mode == "controlnet_important":
            uc = {"c_concat": [conditions[i*nSample: i*nSample+nSample]], "c_crossattn": [uc_cross], "image_control_style": cond_img_style_cat}
        else:
            uc = {"c_concat": [conditions[i*nSample: i*nSample+nSample]], "c_crossattn": [uc_cross], "image_control": cond_img_cat, "image_control_style": cond_img_style_cat}
        ...
        if 'extra_appearance' in batch_data:
            ...
            c['more_image_control_style'] = [more_cond_imgs_style]
        ...
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
            ...
            motion_attn_mode = "none"

            c['image_control_style'] = cond_img_style_cat
            c['more_image_control_style'] = [more_cond_imgs_style]
            c['batch_index'] = i 
            # c['c_depth'] = depth_map

            gene_img, intermediates = infer_model.sample_log(cond=c,
                                    batch_size=nSample, ddim=True,
                                    ddim_steps=50, eta=0.5,
                                    unconditional_guidance_scale=3,
                                    unconditional_conditioning=uc,
                                    inpaint=inpaint,
                                    style_timestep=args.style_timestep,
                                    motion_attn_bank=motion_attn_bank,
                                    motion_attn_mode=motion_attn_mode,
            ...
            for j in range(nSample):
                ...
                if not args.save_video:
                    save_image(gene_img[j].cpu().squeeze().clamp(-1, 1).add(1).mul(0.5), f"{args.local_image_dir}/{image_name}/{str(i*nSample+j).zfill(2)}.jpg")
...
def visualize_all(args, name, batch_data, infer_model, case, nSample):
    infer_model.eval()
    # video length #max(nSample, batch_data["image"].squeeze().shape[0])
    ...
    # conditions = torch.roll(conditions, shifts=3, dims=0)
    # fea_condtion = torch.roll(fea_condtion, shifts=3, dims=0)  
    ...
    if args.denoise_from_guidance:
        #import pdb;pdb.set_trace()
        fea_map_enc_all = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(fea_condtion))
        # fea_map_enc = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(fea_condtion[i*nSample: i*nSample+nSample]))
        c = {"c_concat": [conditions], "c_crossattn": [c_cross], "image_control": cond_img_cat, 'feature_control':fea_map_enc_all, "image_control_style": cond_img_style_cat, "style_mask": batch_data['style_mask'].cuda()}
    else:
        c = {"c_concat": [conditions], "c_crossattn": [c_cross], "image_control": cond_img_cat, "image_control_style": cond_img_style_cat, "style_mask": batch_data['style_mask'].cuda()}
    if args.control_mode == "controlnet_important":
        uc = {"c_concat": [conditions], "c_crossattn": [uc_cross], "image_control_style": cond_img_style_cat}
    else:
        uc = {"c_concat": [conditions], "c_crossattn": [uc_cross], "image_control": cond_img_cat, "image_control_style": cond_img_style_cat}
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
        ...
        for j in range(32):
            if 'extra_appearance' in batch_data:
                if 'fea_condition' in batch_data and args.denoise_from_guidance:
                    cated = torch.cat([fea_condtion[j].cpu().squeeze(),gene_img[j].cpu().squeeze(), conditions[j].cpu().squeeze(), cond_imgs.cpu().squeeze(), batch_data['extra_appearance'].cpu().squeeze()],axis=2)
                else:
                    cated = torch.cat([gene_img[j].cpu().squeeze(), conditions[j].cpu().squeeze(), cond_imgs.cpu().squeeze(), batch_data['extra_appearance'].cpu().squeeze()],axis=2)
            else:
                cated = torch.cat([gene_img[j].cpu().squeeze(), conditions[j].cpu().squeeze(), cond_imgs.cpu().squeeze()],axis=2)
            cated = cated.clamp(-1, 1).add(1).mul(0.5).permute(1,2,0).cpu().numpy()
            #import pdb;pdb.set_trace()
            gene_img_list.append(cated)
            generated_imgs.append(gene_img[j].squeeze().clamp(-1, 1).add(1).mul(0.5).permute(1,2,0).cpu().numpy())
            ...
            if not args.save_video:
                save_image(gene_img[j].cpu().squeeze().clamp(-1, 1).add(1).mul(0.5), f"{args.local_image_dir}/{image_name}/{str(j).zfill(2)}.jpg")
...
def main(args):
    ...
    if args.test_dataset == 'back_head_generation':
       ...
       test_image_dataset = test_dataset_cls(
                                ...
                                style_front_path=args.style_image_front,
                                style_back_path=args.style_image_back,
    ...
    out_files = os.listdir(args.local_image_dir)
    ...
    for itr in range(0, len(test_image_dataloader)):
        ...
        print(f"processing {test_batch_data['image_name'][0]} ...")

        # skip if the output exist
        if (os.path.isdir((f"{args.local_image_dir}/{test_batch_data['image_name'][0].split('.')[0]}"))  and len(os.listdir(f"{args.local_image_dir}/{test_batch_data['image_name'][0].split('.')[0]}")) > 0) or os.path.isfile((f"{args.local_image_dir}/{test_batch_data['image_name'][0]}.mp4")):
            print(f"skip the {test_batch_data['image_name'][0]} since it already exists!")
            continue

        with torch.no_grad():
            nSample = int(args.nSample) # video length 

            image_name = test_batch_data["image_name"][0].split('.')[0]

            if image_name in out_files:   
                print(f"skip {image_name} since it has already processed")
                continue

            """
            cond_style_img_path = args.style_image_front
            cond_style_img_back_path = args.style_image_back

            cond_style_img = read_image(cond_style_img_path)
            cond_style_img = image_transform(cond_style_img)
            cond_style_img = cond_style_img.unsqueeze(0)

            cond_style_img_back = read_image(cond_style_img_back_path)
            cond_style_img_back = image_transform(cond_style_img_back)
            cond_style_img_back = cond_style_img_back.unsqueeze(0)

            test_batch_data["condition_image_style"] = cond_style_img
            test_batch_data["extra_appearance_style"] = cond_style_img_back
            """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style_timestep", type=int, default=0, help="The resume directory where the condtiion path will be loaded.")
    parser.add_argument("--style_image_front", type=str, default=None, help="The path of the style image front.")
    parser.add_argument("--style_image_back", type=str, default=None, help="The path of the style image back.")
    parser.add_argument("--video_guidance", action='store_true', default=False, help="Use video guidance or not.")
    parser.add_argument("--save_video", action='store_true', default=False, help="Save video or not.")
    parser.add_argument('--style_mask', type=str, default=None, help='The path of the style mask image')
