...
def load_state_dict(model, ckpt_path, strict=True, map_location="cpu"):
    ...
    # Step 1: Collect new keys to add
    additional_keys = {}
    for key, value in state_dict.items():
        if key.startswith('control_model.'):
            new_key = key.replace('control_model.', 'style_control_model.')
            if new_key not in state_dict:
                additional_keys[new_key] = value
                print(f"Copying {key} to {new_key}")

    # Step 2: Update state_dict with new keys
    state_dict.update(additional_keys)
...
def get_cond_control(args, batch_data, control_type, device, model=None, batch_size=None, train=True):
    ...
    if control_type == "GAN_Generated" :
        if train:
            ...
            condition = torch.stack(batch_data["condition"]).squeeze().cuda()
            c_cat = condition
            ...
            cond_img_cat = cond_img_cat.repeat(condition.shape[0],1,1,1)

            print("Condition shape: ", condition.shape, "cond_img_cat shape: ", cond_img_cat.shape)
        else:
            #assert "pose_map" in batch_data
            condition = torch.stack(batch_data["condition"]).squeeze().cuda() # 1 , B, C , H , W
            c_cat = condition
            cond_image = batch_data["condition_image"].cuda() # 1, C, H , W
            #import pdb;pdb.set_trace()
        ...
        if 'extra_appearance' in batch_data:
            ...
            m_cond_img = m_cond_img.repeat(condition.shape[0], 1, 1, 1)
...
def visualize(args, name, batch_data, tb_writer, infer_model, global_step, nSample, nTest=1,seg_model=None):
    ...
    more_style_list = [more_img.cuda() for more_img in batch_data["extra_appearance_style"]]
    text = batch_data["text_blip"]
    c_cat_list, cond_img_cat, more_cond_imgs = get_cond_control(args, batch_data, args.control_type, args.device, model=infer_model, train=False)
    

    cond_img_cat = [cond_img_cat[0][:nSample].cuda()]
    if more_cond_imgs is not None:
        more_cond_imgs = more_cond_imgs[:nSample].cuda()

    cond_img_style = batch_data['condition_image_style'].cuda() if 'condition_image_style' in batch_data else None
    if cond_img_style is not None:
        cond_img_style = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(cond_img_style))
        cond_img_style = cond_img_style.repeat(nSample, 1, 1, 1)
    
    more_img_style = batch_data['extra_appearance_style'].cuda() if 'extra_appearance_style' in batch_data else None
    more_img_style_list = []
    if more_img_style is not None:
        more_img_style = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(more_img_style))
        more_img_style = more_img_style.repeat(nSample, 1, 1, 1)
        more_img_style_list.append(more_img_style)
    ...
    #import pdb;pdb.set_trace()
    for i in range(target_imgs.shape[0] // nSample):
        print("Generate Image {} in {} images".format((i+1) * nSample, target_imgs.shape[0]))

        print("target_imgs shape: ", target_imgs.shape)
        print("c_cat_list shape: ", len(c_cat_list), c_cat_list[0].shape)
        print("cond_img_cat shape: ", cond_img_cat[0].shape)
        print("more_cond_imgs shape: ", more_cond_imgs.shape if more_cond_imgs is not None else "None")
        #import pdb;pdb.set_trace()
        c_concat = [c_cat_list[0][i*nSample : i*nSample+nSample].cuda()]
        print("c_concat shape: ", c_concat[0].shape)

        fea_map_enc = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(target_imgs[i*nSample: i*nSample+nSample]))
        c = {"c_concat": c_concat, "c_crossattn": [c_cross], "image_control": cond_img_cat, "image_control_style": [cond_img_style], 'feature_control': fea_map_enc}
        if args.control_mode == "controlnet_important":
            uc = {"c_concat":  c_concat, "c_crossattn": [uc_cross], "image_control_style": [cond_img_style]}
        else:
            uc = {"c_concat":  c_concat, "c_crossattn": [uc_cross], "image_control":cond_img_cat, "image_control_style": [cond_img_style]}
        if args.wonoise:
            c['wonoise'] = True
            uc['wonoise'] = True
        else:
            ...
            uc['wonoise'] = False
        if 'extra_appearance' in batch_data:
            more_cond_imgs = batch_data['extra_appearance']
            more_cond_imgs = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(more_cond_imgs.cuda()))
            more_cond_imgs = more_cond_imgs.repeat(nSample, 1, 1, 1)

            print("more_cond_imgs shape: ", more_cond_imgs.shape)
            c['more_image_control'] = [more_cond_imgs]
        if 'extra_appearance_style' in batch_data:
            more_img_style = batch_data['extra_appearance_style']
            more_img_style = infer_model.get_first_stage_encoding(infer_model.encode_first_stage(more_img_style.cuda()))
            more_img_style = more_img_style.repeat(nSample, 1, 1, 1)

            print("more_img_style shape: ", more_img_style.shape)
            c['more_image_control_style'] = [more_img_style]
        ...
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
            ...
            gene_img = gene_img.clamp(-1, 1)
            gene_img_list.append(gene_img.cpu())
    ...
    cated = torch.cat([image.cpu()] + 
                      [c_cat_list[0].cpu()] + 
                      gene_img_list + 
                      [batch_data['condition_image'].cpu()] +
                      [more_img.cpu()[None,...] for more_img in more_image_list] +
                      [batch_data['condition_image_style'].cpu()]  +
                      [more_img_style.cpu()[None,...] for more_img_style in more_style_list]
                      )
...
def main(args):
    ...
    if args.mm_path_dir is None and hexists(os.path.join(args.resume_dir, "optimizer_state_latest.th")):
        ...
        if args.resume_dir is not None and hexists(os.path.join(args.resume_dir, "optimizer_state_latest.th")):
            ...
            opt_state_path = args.optimizer_state_path
            global_step = 0
            print('------ Loading model state dict ------')
            load_state_dict(model, ckpt_path, strict=False) 
            #load optimizer state dict
            if opt_state_path is not None and hexists(opt_state_path):
                print('------ Loading optimizer state dict ------')
                optimizer_state_dict = load_from_pretrain(opt_state_path, map_location="cpu")
                global_step = optimizer_state_dict["step"]
                print(f"Global step set to {global_step} from optimizer state dict.")
            torch.cuda.empty_cache()
    ...
    if args.train_dataset == "full_head_clean_real_data_temporal":
        ...
        image_dataset = dataset_cls(
                                ...
                                inference_image_dataset = args.dataset_root_path,
    ...
    if args.finetune_all:
        ...
        if args.finetune_mmonlypose:
            ...
            params += params_pose
        if args.finetune_mmonlyimage:
            for pd in model.style_control_model.parameters():
                pd.requires_grad_(True)
            params_image = list(model.style_control_model.parameters())
            print("Train appearance layers in UNet")
            params += params_image
        print("Train Motion Module") 
        #import pdb;pdb.set_trace()
    elif args.finetune_styleonly:
        for b in list(model.pose_control_model.input_blocks) + [model.pose_control_model.middle_block] + list(model.pose_control_model.input_hint_block) + list(model.pose_control_model.middle_block_out)  + list(model.pose_control_model.zero_convs):
            for p in b.parameters():
                p.requires_grad_(False)
        for b in list(model.model.diffusion_model.input_blocks) + [model.model.diffusion_model.middle_block] + list(model.model.diffusion_model.output_blocks) + list(model.model.diffusion_model.out):
        ...
        for p in model.control_model.parameters():
            p.requires_grad_(False)
        for p in list(model.model.diffusion_model.input_blocks_motion_module.parameters()) + list(model.model.diffusion_model.output_blocks_motion_module.parameters()):
            p.requires_grad_(False)

        for pd in model.style_control_model.parameters():
            pd.requires_grad_(True)
        params = list(model.style_control_model.parameters())
        print("Train style controlnet")
    else:
        for b in list(model.model.diffusion_model.input_blocks) + [model.model.diffusion_model.middle_block] + list(model.model.diffusion_model.output_blocks) + list(model.model.diffusion_model.out):
            for p in b.parameters():
                p.requires_grad_(False)
    ...
    if args.local_rank == 0:
        for name, param in model.named_parameters():
            if param.requires_grad:
                # write to txt file
                with open(os.path.join(args.local_log_dir, "trainable_params.txt"), "a") as f:
                    f.write(f"{name}\n")
    ...
    for itr in range(global_step, args.num_train_steps):
        ...
        img = batch_data["image"]  #256, -1~1

        start_idx = random.randint(0, num_frames - 1)
        interval = random.randint(2, 6)

        frames_list = [(start_idx + i * interval) % num_frames for i in range(nSample)]
        print("frames_list:", frames_list)
        frames_tensor = torch.tensor(frames_list, device=img.device)

        with torch.no_grad():
            image = img[:, frames_tensor, :, :, :].to(args.device) # B, C, H, W
            ...
            # get nsample condition
            c_cat_list = [c_cat_list[0][frames_tensor]]
            cond_img_cat = [cond_img_cat[0][frames_tensor]]
            if more_image_cond is not None:
                more_image_cond = more_image_cond[frames_tensor]
            ...
            if cond_img_style is not None:
                ...
                print("cond_img_style shape:", cond_img_style.shape)
            ...
            print("c_cat_list shape:", c_cat_list[0].shape)
            print("cond_img_cat shape:", cond_img_cat[0].shape)

            c_cross = infer_model.get_learned_conditioning(text)
            c_cross = c_cross.repeat(x.shape[0], 1, 1)
            cond = {"c_concat": c_cat_list, "c_crossattn": [c_cross], "image_control": cond_img_cat, "more_image_control": [more_image_cond], "image_style": [cond_img_style], "more_image_style": more_img_style_list}
