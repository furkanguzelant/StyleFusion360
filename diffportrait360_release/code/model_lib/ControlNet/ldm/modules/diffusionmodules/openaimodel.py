...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, banks=None, attention_mode=None, attn_index=None, uc=False, style_attn_banks=None, style_write=False, index=0, motion_attn_bank=None, motion_attn_mode=None, batch_index=0, style_mask=None):
        ...
        for layer in self:
            ...
            if isinstance(layer, TimestepBlock):
                ...
                if uc:
                    ...
                    # pdb.set_trace()
                    x = layer(x, context, banks, attention_mode, attn_index, style_attn_banks=style_attn_banks, style_write=style_write, index=index, style_mask=style_mask)
                    if attention_mode == 'read' or attention_mode == 'read_write':
                        attn_index+=1    
            elif isinstance(layer, VanillaTemporalModule):
                # print("layer Motion Module")
                # pdb.set_trace()
                x = layer(x, context, attn_bank=motion_attn_bank, attn_mode=motion_attn_mode, batch_index=batch_index)
            else:
                # print("layer others")
                # pdb.set_trace()
                x = layer(x)
...
class UNetModel_Temporal(nn.Module):
    ...
    def __init__(
        ...
        self.style_attention_scale = self.unet_additional_kwargs['style_attention_scale']
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_transformer,
                                use_checkpoint=use_checkpoint, style_attention_scale=self.style_attention_scale
                            )
                        )
