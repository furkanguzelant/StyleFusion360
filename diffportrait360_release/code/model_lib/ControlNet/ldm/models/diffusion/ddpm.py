...
class LatentDiffusionReferenceOnly(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        image_style = None
        if 'image_style' in cond and cond['image_style'] is not None:
            print("Using image style conditioning")
            image_style = cond['image_style']
            image_style = torch.cat(image_style, dim=1)

        more_style_image_noisy = [] 
        if 'more_image_style' in cond and cond['more_image_style'] is not None:
            num_additional_ref_imgs = len(cond['more_image_style'])
            for i in range(num_additional_ref_imgs):
                m_style_img_noisy = cond['more_image_style'][i]
                if not cond['wonoise']:
                    m_style_img_noisy = self.model.q_sample(m_style_img_noisy, t)
                more_style_image_noisy.append(m_style_img_noisy)
        
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        #import pdb;pdb.set_trace()
        model_output = self.apply_model(x_noisy, t, cond, reference_image_noisy, more_reference_image_noisy = more_reference_image_noisy, reference_image_noisy2=image_style, more_reference_image_noisy2= more_style_image_noisy)
